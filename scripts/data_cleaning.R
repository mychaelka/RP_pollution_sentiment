...
# join sentiment measures and topics into one data frame
...
# remove unnecessary columns and convert to spatial dataframe
...
# load shapefile with counties for cropping, remove Alaska and Hawaii
...
# intersect twitter data with US counties
sf_use_s2(FALSE)
data <- st_intersection(data, usa_counties)

# helper function to convert character values with units into numbers (for pollution data)
...
# same as above but for weather data (it has columns ordered differently than pollution)
...
# load pollution and weather data and extract individual variables
...
# function to adjust column names -- midnight by default does not include time value
...
# save spatial df into new variables
wind_xgeo <- wind_x
wind_ygeo <- wind_y
...
# remove spatial dimensions and convert to tibble
...
# helper function to create long format of df
...
# helper function for interpolating hourly values 
# inside the function, change variable on lines 6 and 9 to what you currently need 
# (dewpoint, temperature, pm25,...). Must be done for all needed variables.
...
# interpolation, must be done individually after changing the function above each time
wind_x_interpolated <- 
  wind_x %>% 
  split(.$id) %>% 
...
# join twitter data with pm25 spatial dataframe and interpolated values
full_data <- 
  pm25geo %>% 
  dplyr::select(id) %>% 
